...
describe('BridgeContract', () => {
  ...
  describe('#constructor', () => {
    it('fails with network undefined', () => {
      expect(
        () =>
          new BridgeContract(networkByChainID(0), ADDRESS_ONE, ADDRESS_TWO)
            .bridgeContractAddress
      ).toThrow('NETWORK_UNDEFINED');
    });

    it('fails with same addresses', () => {
      expect(
        () =>
          new BridgeContract(network, ADDRESS_ONE, ADDRESS_ONE)
            .bridgeContractAddress
      ).toThrow('DUPLICATE_ADDRESS');
    });
